...
public class Loader {
	...
	public void addInstanceAttribute(int vao, int vbo, int attribute, int dataSize,
			int instanceDataLength, int offset) {
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
		GL30.glBindVertexArray(vao);
		GL20.glVertexAttribPointer(attribute, dataSize, GL11.GL_FLOAT, false, instanceDataLength * 4, 
				offset * 4);
	...
	public int loadTexture(String fileName) {
		...
		try {
			...
			if(GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic) {
				...
				GL11.glTexParameterf(GL11.GL_TEXTURE_2D, EXTTextureFilterAnisotropic.
						GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, amount);
			}else {
	...
	public int loadCubeMap(String[] textureFiles) {
		...
		for(int i=0; i < textureFiles.length; i++) {
			...
			GL11.glTexImage2D(GL13.GL_TEXTURE_CUBE_MAP_NEGATIVE_X + i, 0, GL11.GL_RGBA, data.getWidth(), 
